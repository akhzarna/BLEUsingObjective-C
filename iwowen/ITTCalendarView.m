...
- (void)initParameters
{
    ...
    __selectedDayDic = [[NSMutableDictionary alloc] init];
...
- (NSString*)findMonthDescription
{
    ...
    if (!title||![title length]) {        
        title = [NSString stringWithFormat:@"%d, %d", [_calMonth getMonth], [_calMonth getYear]];
    }
    return title;
}
...
- (NSArray*)findWeekTitles
{
    ...
    if (!titles||![titles count]) {
        titles = [NSArray arrayWithObjects:@"S", @"M", @"T", @"W", @"T", @"F", @"S", nil];
...
- (BOOL) isHistorySelectedDay:(ITTCalDay *)calDay
{
    @try {
        ...
        return ([__selectedDayDic objectForKey:key] != nil);
    }
        ...
        for (NSInteger column = 0; column < NUMBER_OF_DAYS_IN_WEEK; column++) {
            ...
            if (gridView.selected ^ selected) {
                ...
                if (day >0 && day <= 31) {
                    ...
                    if (selected)
                    {
                        [__selectedDayDic setObject:@"1" forKey:key];
                    }
                    else
                    {
                        [__selectedDayDic removeObjectForKey:key];
                    }                    
                }
            }
        }
...
- (void)layoutGridCells
{
    ...
    if ([calDay getWeekDay] > 1) {
        ...
        row = 0;
        for (NSInteger day = previousMonth.days; count > 1 && day >= 1; day--) {
...
//        if(day == 5){
//            NSString *key = [NSString stringWithFormat:@"%d%d", row, column];
//            _selectedGridViewIndicesDic[key] = [NSString stringWithFormat:@"%d", 1];
//        }
        ...
        gridView.row = row;
        gridView.column = column;
        gridView.selectedEanable = TRUE;//([self isEarlyerMinimumDay:calDay] || [self isAfterMaximumDay:calDay]) ? FALSE:TRUE;
        if ([self isHistorySelectedDay:calDay]) {
            hasSelectedDay = TRUE;
            gridView.selected = TRUE;
            [self setSelectedAtRow:row column:column selected:TRUE];
        }else{
               [self setSelectedAtRow:row column:column selected:FALSE];
//            NSLog(@"day (%lu) is not selected",(unsigned long)calDay.getDay);
        }
        frame = [self getFrameForRow:row column:column];
        gridView.frame = frame;
        [gridView setNeedsLayout];
...
- (void)dealloc 
{
    ...
    [__selectedDayDic release];
    __selectedDayDic = nil;
        ...
        for (NSUInteger row = 0; row < rows; row++) {
            for (NSUInteger column = 0; column < NUMBER_OF_DAYS_IN_WEEK; column++) {
                if ([self isSelectedAtRow:row column:column]) {
                    ...
                    NSLog(@"selected row %d column %d day %d", row, column, day);
                }
            }
        }
...
- (void)calendarSrollViewTouchesBegan:(ITTCalendarScrollView*)calendarScrollView touches:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    NSLog(@"calendarSrollViewTouchesBegan");
}
...
//        if (!_allowsMultipleSelection) {
    ...
    NSLog(@"calendarSrollViewTouchesEnded");
    if (_delegate && [_delegate respondsToSelector:@selector(calendarViewDidSelectDay:calDay:)]) {
        [_delegate calendarViewDidSelectDay:self calDay:self.selectedDay];
...
#pragma mark - Ali Changes

-(void)setselectedDayDictionary:(NSMutableDictionary*)dict{
    
    [self recyleAllGridViews];
    _selectedGridViewIndicesDic = [[NSMutableDictionary alloc] init];
    __selectedDayDic = [[NSMutableDictionary alloc]init];
    for (NSString *aKey in [dict allKeys]) {
        [__selectedDayDic setValue:[dict objectForKey:aKey] forKey:aKey];
    }
